...
% ## Nozzle Engineering Technical Drawing
%
% This LaTeX code uses the **TikZ** library to produce a precise, professional-grade technical drawing of a **supersonic nozzle** (specifically a De Laval nozzle) in cross-section. It is designed to emulate standard mechanical engineering blueprints, featuring automated dimensioning and industrial hatch patterns.
%
% ---
%
% ### Key Features
%
% * **Automated Dimensioning:** Defines custom TikZ decorations (`Hdim`, `Vdim`, and `dim`) to create technical dimension lines with arrows, offset distances, and text labels (e.g., diameters and radii).
% * **Custom Section Hatching:** Implements a custom PGF pattern called `section` to simulate the standard diagonal cross-hatching used in engineering drawings to indicate solid material.
% * **Geometric Precision:** Utilizes the `calc` and `intersections` libraries to accurately place coordinates for the nozzle throat, flanges, and screw holes.
% * **Symmetric Rendering:** Employs a `\foreach` loop to mirror the upper and lower halves of the nozzle, ensuring perfect symmetry across the central axis.
%
% ### Technical Components
%
% * **Nozzle Profile:** Visualizes the convergent-divergent transition, including specific radius arcs (`R7`) at the throat.
% * **Flange Details:** Includes a mounting flange with detailed screw hole placements and centerlines.
% * **Annotations:** Provides full measurement callouts in millimeters, including diameter symbols ($\diameter$) and angle markers (\ang{90}).
...
\else
	...
	% dimension overline II and it depends on the
	% quadrant where the the segment is.
	\pgfmathparse{abs(\dim@sep)\pgfdecoratedangle
		% Third quadrant
		...
		% dimension.
		\pgfmathparse{abs(.5\dim@y)\pgfdecoratedangle
		% First quadrant
		% Node drawing. It can be translate along the path with
		% text translation option.
		{
