...
describe('HttpController.agentReplace', function () {
  ...
  it('returns 204 without touching the doc when old_text === new_text', async function () {
    ...
    sinon.assert.notCalled(this.DocumentManager.promises.getDocWithLock)
    sinon.assert.notCalled(this.UpdateManager.promises.applyUpdate)
  })

  it('returns 404 when old_text is not found', async function () {
