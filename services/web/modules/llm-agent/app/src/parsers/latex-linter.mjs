// @ts-check
//
// Ported from services/web/frontend/js/features/source-editor/languages/latex/linter/latex-linter.worker.ts
// Upstream sha: 1b5887d97f9b4360cd338ff603cedcb624040749
//
// Refresh procedure:
//   git diff <old-sha>..<new-sha> -- services/web/frontend/js/features/source-editor/languages/latex/linter/latex-linter.worker.ts
// Re-apply non-type-annotation changes here, bump the sha above.
//
// Backend port: types stripped via babel preset-typescript (preserves line
// numbers vs upstream for grep parity); the Worker onmessage/postMessage
// wiring and dummy LintWorker class at the bottom were removed — instead we
// export Parse(text) directly so SyntaxChecker can call it inline.
//
// Note: 'document' inside this file is a *local* variable (a DocumentTree
// instance built from tokens), not the browser DOM. The naming collision
// with browser globals is harmless on the backend.
