...
//
// Ported from services/web/frontend/js/ide/human-readable-logs/HumanReadableLogsRules.tsx
// Upstream sha: 1b5887d97f9b4360cd338ff603cedcb624040749
//
// Refresh procedure:
//   git diff <old-sha>..<new-sha> -- services/web/frontend/js/ide/human-readable-logs/HumanReadableLogsRules.tsx
// Re-apply non-type-annotation changes here, bump the sha above.
//
// Backend port: types stripped. The two upstream `improvedTitle` callbacks
// return [string, JSX.Element]; the JSX half is already discarded by
// HumanReadableLogs.ts (see comment at upstream line ~50), so we drop the
// JSX entirely and return only the string. No functional difference.

import {
  packageSuggestionsForCommands,
  packageSuggestionsForEnvironments,
} from './HumanReadableLogsPackageSuggestions.mjs'
...
const rules = [
  ...
  },
  {
    ...
    improvedTitle: (currentTitle, details) => {
      ...
      if (suggestion) {
        return `Is ${suggestion.command} missing?`
      }
      return currentTitle
    },
